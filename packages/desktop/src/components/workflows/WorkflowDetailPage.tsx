...
const WORKFLOW_DESCRIPTIONS: Record<string, string> = {
  ...
  'content-creation-pipeline': 'End-to-end content production from ideation to publishing. Generates topic ideas based on trending keywords, drafts long-form articles with SEO optimization, creates social media snippets, and queues everything for review — all orchestrated by AI agents working together.',
  'workflow-creator': 'Build and publish your own custom workflows from scratch. Define agents, scripts, connectors, and scheduling — then package it all into a shareable workflow that others can install with one click.',
  'customer-support-automation': 'Triage and respond to incoming support tickets automatically. Categorizes issues by urgency and type, drafts contextual replies using your knowledge base, escalates complex cases to the right team member, and tracks resolution metrics in real time.',
}

function Chip({ label }: { label: string }) {
  return (
    <span style={{
      fontSize: '13px', color: '#c8c8cc',
...
function InstallModal({
  ...
  const allReady = connectorCheck && connectorCheck.workflowId === workflow.id && connectorCheck.missing.length === 0
      ...
      >
        ...
        >
          ...
          <div style={{
            ...
            <p style={{ fontSize: '12px', fontWeight: 500, color: '#a1a1aa', margin: '0 0 8px' }}>What happens next:</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
              {[
                ...
                <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: '8px', fontSize: '12px', color: '#71717a' }}>
          ...
          {/* Connection check */}
          {loading ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '12px 0', fontSize: '13px', color: '#71717a' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {connectorCheck!.satisfied.map((c) => (
                <div key={c} style={{ display: 'flex', alignItems: 'center', gap: '10px', fontSize: '13px' }}>
              ...
              {connectorCheck!.optional.map((c) => (
                <div key={c.id} style={{ display: 'flex', alignItems: 'center', gap: '10px', fontSize: '13px' }}>
                  <span style={{ color: c.connected ? '#34d399' : '#52525b', fontSize: '15px' }}>{c.connected ? '\u2713' : '\u25CB'}</span>
                  <span style={{ color: '#d4d4d8' }}>{c.id}</span>
                  <span style={{ color: '#52525b' }}>{c.connected ? 'connected' : 'not connected'} (optional)</span>
                </div>
              ))}
            </div>
          )}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginTop: '24px' }}>
            {allReady ? (
              ...
              >
                {installing && <Loader2 size={14} strokeWidth={2} style={{ animation: 'spin 1s linear infinite' }} />}
      ...
      <div style={{ maxWidth: '640px', margin: '0 auto', padding: '20px 0 40px' }}>
        ...
        <button
          ...
          style={{
            ...
            fontSize: '13px', color: '#71717a', cursor: 'pointer',
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '16px' }}>
          <div style={{ flex: 1 }}>
            <h1 style={{ fontSize: '28px', fontWeight: 700, color: '#e4e4e7', margin: 0, lineHeight: 1.2 }}>
              {workflow.name}
            </h1>
            <p style={{ fontSize: '14px', color: '#71717a', marginTop: '6px', margin: '6px 0 0' }}>
              by {workflow.author}
            </p>
          </div>
          {installed ? (
            <div style={{
              ...
              fontSize: '14px', fontWeight: 500, color: '#34d399',
            ...
            <button
              ...
              style={{
                ...
                cursor: 'pointer', whiteSpace: 'nowrap', marginTop: '4px',
              }}
            >
              Install Workflow
            </button>
        ...
        {/* Description */}
        <p style={{ fontSize: '15px', lineHeight: '1.65', color: '#a1a1aa', marginTop: '20px', maxWidth: '680px' }}>
        ...
        {agents.length > 0 && (
          <div style={{ marginTop: '32px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <h3 style={{ fontSize: '15px', fontWeight: 600, color: '#e4e4e7', margin: 0 }}>Agents</h3>
              <span style={{ fontSize: '12px', color: '#71717a', background: 'rgba(255,255,255,0.07)', padding: '2px 8px', borderRadius: '6px' }}>{agents.length}</span>
            </div>
        ...
        {scripts.length > 0 && (
          <div style={{ marginTop: '24px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <h3 style={{ fontSize: '15px', fontWeight: 600, color: '#e4e4e7', margin: 0 }}>Scripts</h3>
              <span style={{ fontSize: '12px', color: '#71717a', background: 'rgba(255,255,255,0.07)', padding: '2px 8px', borderRadius: '6px' }}>{scripts.length}</span>
            </div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginTop: '12px' }}>
              {scripts.map((s) => <Chip key={s} label={s} />)}
            </div>
          </div>
        )}
        ...
        <div style={{ marginTop: '24px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <h3 style={{ fontSize: '15px', fontWeight: 600, color: '#e4e4e7', margin: 0 }}>Connectors</h3>
            <span style={{ fontSize: '12px', color: '#71717a', background: 'rgba(255,255,255,0.07)', padding: '2px 8px', borderRadius: '6px' }}>{workflow.connectors.length}</span>
          </div>
          <p style={{ fontSize: '13px', color: '#52525b', marginTop: '6px' }}>
            Services this workflow connects to.
          </p>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginTop: '10px' }}>
            {workflow.connectors.map((c) => <Chip key={c} label={c} />)}
          </div>
        </div>
