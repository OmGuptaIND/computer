...
function getActionIcon(toolName: string, toolInput?: Record<string, unknown>): React.ElementType {
  ...
  switch (toolName) {
    case 'shell': {
      ...
      if (cmd.startsWith('npm') || cmd.startsWith('pnpm') || cmd.startsWith('yarn') || cmd.startsWith('pip'))
        return Settings2
      if (cmd.startsWith('git')) return GitBranch
      if (cmd.startsWith('curl') || cmd.startsWith('wget')) return Globe
      if (cmd.startsWith('cp') || cmd.startsWith('mv') || cmd.startsWith('mkdir'))
        return FolderOpen
      return Terminal
    }
    case 'filesystem':
  ...
  switch (toolName) {
    ...
    case 'filesystem': {
      ...
      switch (op) {
        ...
        case 'write': case 'create': return `Writing to ${shortPath}`
        case 'list': return `Listing ${shortPath}`
        case 'tree': return `Exploring ${shortPath}`
        case 'search': return `Searching in ${shortPath}`
        case 'delete': return `Deleting ${shortPath}`
        default: return `${op || 'Operating on'} ${shortPath}`
      }
    }
    case 'browser': {
      const op = toolInput.operation as string
      switch (op) {
        case 'open': return `Opening ${((toolInput.url as string) || '').slice(0, 50)}`
        case 'click': return `Clicking ${toolInput.ref || 'element'}`
        case 'fill': return `Typing in ${toolInput.ref || 'input'}`
        case 'snapshot': return 'Reading page elements'
        case 'screenshot': return 'Capturing screenshot'
        case 'scroll': return `Scrolling ${toolInput.direction || 'page'}`
        case 'close': return 'Closing browser'
        case 'fetch': return `Fetching ${((toolInput.url as string) || '').slice(0, 50)}`
        case 'extract': return `Extracting from page`
        default: return `Browser: ${op}`
      }
    }
    case 'git': {
      const op = toolInput.operation as string
      switch (op) {
        case 'clone': return `Cloning ${toolInput.url || 'repository'}`
        case 'commit': return `Committing: ${(toolInput.message as string)?.slice(0, 50) || 'changes'}`
        case 'push': return 'Pushing to remote'
        case 'pull': return 'Pulling from remote'
        case 'status': return 'Checking git status'
        default: return `Git ${op}`
      }
    }
    case 'process':
...
function ToolResultBlock({ content, isError }: { content: string; isError: boolean }) {
  ...
  return (
    <div className={`tool-result-inline${isError ? ' tool-result-inline--error' : ''}`}>
      ...
      >
        {isError ? (
          <XCircle size={13} strokeWidth={1.5} className="tool-result-inline__status-icon tool-result-inline__status-icon--error" />
        ) : (
          <CheckCircle size={13} strokeWidth={1.5} className="tool-result-inline__status-icon tool-result-inline__status-icon--success" />
        )}
