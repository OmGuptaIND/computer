...
import { getDefaultMachine, loadMachines, saveMachine } from '../lib/machines.js'
 ...
 */
async function quickConnect(machine: {
  name: string
  host: string
  port: number
  token: string
  ...
  console.log()
  console.log(
    `  ${ICONS.connecting} ${theme.dim(`${machine.name}`)} ${theme.dim(`(${proto}://${machine.host}:${machine.port})`)}`,
  )
  ...
  try {
    ...
    console.log(
      `  ${ICONS.disconnected} ${theme.error('Connection failed:')} ${theme.dim((err as Error).message)}`,
    )
    console.log(
      `  ${theme.dim('Run')} ${theme.bold('anton connect <host> --token <token>')} ${theme.dim('to reconfigure.')}`,
    )
    console.log()
    process.exit(1)
  }
 ...
 */
async function connectAndLaunch(machine: {
  name: string
  host: string
  port: number
  token: string
  ...
  console.log()
  console.log(
    `  ${ICONS.connecting} Connecting to ${theme.bold(`${proto}://${machine.host}:${machine.port}`)}...`,
  )
  ...
  try {
    ...
    console.log(
      `  ${ICONS.toolDone} Saved as ${theme.bold(`"${machine.name}"`)}${isFirst ? theme.dim(' (default)') : ''}`,
    )
    conn.disconnect()
    ...
    console.log(
      `    ${theme.dim('• Is the agent running?')} ${theme.muted('ssh into your VPS and check')}`,
    )
    console.log(
      `    ${theme.dim('• Is port')} ${theme.bold(String(machine.port))} ${theme.dim('open in your firewall/security group?')}`,
    )
    if (!machine.useTLS) {
