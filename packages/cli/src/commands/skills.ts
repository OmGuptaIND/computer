...
export async function skillsCommand(action: 'list' | 'run', skillName?: string): Promise<void> {
  ...
  if (action === 'list') {
    ...
    console.log(
      `  ${theme.dim('Use')} ${theme.bold("anton chat 'list your skills'")} ${theme.dim('to see active skills.')}\n`,
    )
    return
  }
  ...
  if (action === 'run' && skillName) {
    ...
    console.log(
      `  ${theme.dim('Use')} ${theme.bold(`anton chat 'run skill: ${skillName}'`)} ${theme.dim('for now.')}\n`,
    )
    return
  }
