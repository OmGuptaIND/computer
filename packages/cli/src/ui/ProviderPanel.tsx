  ...
  return (
    <Box flexDirection="column" borderStyle="round" borderColor="#FF6B35" paddingX={2} paddingY={1}>
      ...
      {providers.map((p, i) => {
        ...
        const keyHint = p.hasApiKey ? (p.baseUrl ? p.baseUrl : 'configured') : '(no key)'

        return (
          <Box key={p.name}>
            <Text color={isSelected ? '#FF6B35' : undefined}>
              {cursor} {keyStatus} {p.name.padEnd(14)} <Text dimColor>{keyHint}</Text>
      ...
      {editingKey ? (
        ...
        <Text dimColor>[↑↓] navigate [e] edit key [Esc] close</Text>
      )}
    </Box>
  )
}
