...
import { NotionAPI, blockToText, getPageTitle } from './api.js'
  ...
  return [
    defineTool({
      ...
      parameters: Type.Object({
        ...
        filter: Type.Optional(Type.String({ description: '"page" or "database" to filter results type' })),
      }),
      async execute(_id, params) {
        try {
          const filter = params.filter === 'page' || params.filter === 'database'
    ...
    defineTool({
      ...
      async execute(_id, params) {
        try {
          const parent = params.parent_type === 'database'
    ...
    defineTool({
      ...
      parameters: Type.Object({
        ...
        filter_json: Type.Optional(Type.String({ description: 'Filter object as JSON string (Notion filter format)' })),
      ...
      async execute(_id, params) {
        try {
          ...
          const rows = result.results.map((r) => ({
            ...
            properties: Object.entries(r.properties).reduce<Record<string, unknown>>((acc, [key, val]) => {
              const v = val as { type?: string; [k: string]: unknown }
              if (v.type === 'title') acc[key] = getPageTitle(r)
              else if (v.type === 'rich_text') acc[key] = (v.rich_text as Array<{ plain_text: string }>)?.map((t) => t.plain_text).join('')
              else if (v.type === 'number') acc[key] = v.number
              else if (v.type === 'select') acc[key] = (v.select as { name?: string } | null)?.name
              else if (v.type === 'multi_select') acc[key] = (v.multi_select as Array<{ name: string }>)?.map((s) => s.name)
    ...
    defineTool({
      ...
      parameters: Type.Object({
        page_id: Type.String({ description: 'Notion page ID' }),
        properties_json: Type.String({ description: 'Properties to update as JSON string (Notion properties format)' }),
      }),
      async execute(_id, params) {
        try {
