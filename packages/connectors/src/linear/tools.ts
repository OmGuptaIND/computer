...
export function createLinearTools(api: LinearAPI): AgentTool[] {
  return [
    ...
    defineTool({
      ...
      label: 'List Issues',
      description: '[Linear] List issues with optional filters by team, state, assignee, or priority.',
      parameters: Type.Object({
        team_id: Type.Optional(Type.String({ description: 'Filter by team ID (use linear_list_teams to find IDs)' })),
        state_type: Type.Optional(Type.String({ description: 'Filter by state type: backlog | unstarted | started | completed | cancelled' })),
        assignee_id: Type.Optional(Type.String({ description: 'Filter by assignee user ID' })),
        priority: Type.Optional(Type.Number({ description: 'Filter by priority: 0=none, 1=urgent, 2=high, 3=medium, 4=low' })),
      ...
      async execute(_id, params) {
        try {
          ...
          return toolResult(JSON.stringify(issues.map((i) => ({
            id: i.id,
      ...
      async execute(_id, params) {
        try {
          ...
          return toolResult(JSON.stringify({
            ...
            assignee: issue.assignee ? { id: issue.assignee.id, name: issue.assignee.displayName } : null,
    ...
    defineTool({
      ...
      parameters: Type.Object({
        ...
        state_id: Type.Optional(Type.String({ description: 'Workflow state ID (use linear_list_states)' })),
    ...
    defineTool({
      ...
      label: 'Update Issue',
      description: '[Linear] Update an existing issue (title, description, state, priority, assignee, due date).',
      parameters: Type.Object({
        issue_id: Type.String({ description: 'Issue UUID or identifier like ENG-123' }),
        ...
        assignee_id: Type.Optional(Type.String({ description: 'New assignee user ID (empty string to unassign)' })),
        due_date: Type.Optional(Type.String({ description: 'Due date YYYY-MM-DD (empty string to clear)' })),
      }),
      async execute(_id, params) {
        try {
    ...
    defineTool({
      ...
      async execute(_id, params) {
        try {
          ...
          return toolResult(JSON.stringify(comments.map((c) => ({
            id: c.id,
    ...
    defineTool({
      ...
      parameters: Type.Object({
        state_type: Type.Optional(Type.String({ description: 'Filter: backlog | unstarted | started | completed | cancelled' })),
        limit: Type.Optional(Type.Number({ description: 'Max results (default: 25)' })),
      }),
      async execute(_id, params) {
        try {
          const me = await api.getViewer()
          const issues = await api.listIssues({ assigneeId: me.id, stateType: params.state_type, first: params.limit })
